...
func merge(nums1 []int, m int, nums2 []int, n int) {
	...
	for i, j, k := m-1, n-1, m+n-1; j >= 0; k-- {
		// исключаем панику, если элементы в первом массиве все уже взяты (пример как в кейсе 4)
		if i >= 0 && nums1[i] >= nums2[j] { // если элементы есть, тогда сверяем последний с последним элементов другого массива
			nums1[k] = nums1[i] // если первый больше или такой же, то вписываем в конец общей длины это значение
